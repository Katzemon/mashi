...
async def process_trait(trait):
    if is_apng(trait):
        total_duration = await asyncio.to_thread(get_apng_info, trait)
        return total_duration

    elif is_gif(trait):
        total_duration = await asyncio.to_thread(get_gif_info, trait)
        return total_duration

    elif is_webp(trait):
        total_duration = await asyncio.to_thread(get_webp_info, trait)
        return total_duration
    else:
        return ANIM_STEP
